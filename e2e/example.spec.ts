...
test.describe("Homepage", () => {
  ...
  test("should toggle dark mode correctly", async ({ page }) => {
    ...
    // Test: Click theme toggle to open dropdown
    await themeToggle.click();

    // Select dark mode from dropdown (Radix renders radio items as menuitemradio)
  ...
  test("should be keyboard accessible", async ({ page }) => {
    ...
    // Test: Keyboard can activate the toggle and select dark mode
    await themeToggle.click();
    const darkOption = page.getByRole("menuitemradio", { name: /dark/i });
    await darkOption.click();
    await page.waitForTimeout(300);
...
test.describe("Responsive Design", () => {
  test("should render correctly on mobile (375px)", async ({ page }) => {
    ...
    expect(scrollWidth).toBeLessThanOrEqual(clientWidth + 20); // Allow scrollbar/subpixel tolerance
