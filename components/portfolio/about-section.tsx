...
export function AboutSection({ carousel = false }: AboutSectionProps) {
  ...
  if (carousel) {
    const bentoImages = aboutImages.slice(0, 3);
      ...
      >
        ...
        <div className="moog-card flex flex-col justify-center rounded-md p-8">
          ...
          <p className="mt-4 max-w-sm text-sm text-muted-foreground">
            Outside of work, I&apos;m either gigging, playing 5-a-side football,
            or hanging out with my partner and dog.
          </p>
        </div>
        ...
        {bentoImages.map((img) => (
          <div
            key={img.id}
            className="moog-card group relative h-full w-full overflow-hidden rounded-md"
          >
            <Image
              src={img.src}
              alt={img.alt}
